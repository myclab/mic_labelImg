...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        operlistLayout = QVBoxLayout()
        operlistLayout.setContentsMargins(0, 0, 0, 0)
        operlistLayout.addWidget(self.copy_prev_button)
        operlistLayout.addStretch()
        operlistLayout.addSpacing(10)
        operListContainer = QWidget()
        operListContainer.setFixedHeight(40) 
        operListContainer.setLayout(operlistLayout)
        self.operdock = QDockWidget(u'Operation List', self)
        self.operdock.setObjectName(u'Operation')
        self.operdock.setWidget(operListContainer)

        # michaelwan 2019-07-22 add label radio button
        self.pointButton = QRadioButton('Point')
        self.pointButton.clicked.connect(self.point)
        self.lineButton = QRadioButton('Line')
        self.lineButton.clicked.connect(self.line)
        self.squareButton = QRadioButton('Square')
        self.squareButton.clicked.connect(self.square)
        self.circleButton = QRadioButton('Circle')
        self.circleButton.clicked.connect(self.circle)
        self.polygonButton = QRadioButton('Polygon')
        self.polygonButton.clicked.connect(self.polygon)
        shapelistLayout = QVBoxLayout()
        shapelistLayout.setContentsMargins(0, 0, 0, 0)
        shapelistLayout.addWidget(self.pointButton)
        shapelistLayout.addWidget(self.lineButton)
        shapelistLayout.addWidget(self.squareButton)
        shapelistLayout.addWidget(self.circleButton)
        shapelistLayout.addWidget(self.polygonButton)
        shapelistLayout.addStretch()
        shapelistLayout.addSpacing(10)
        self.squareButton.setChecked(True)
        shapeListContainer = QWidget()
        shapeListContainer.setFixedHeight(110) 
        shapeListContainer.setLayout(shapelistLayout)
        self.shapedock = QDockWidget(u'Shape Option', self)
        self.shapedock.setObjectName(u'Shape')
        self.shapedock.setWidget(shapeListContainer)
        ...
        boxListContainer.setFixedHeight(100) 
        self.boxdock = QDockWidget(u'Box Option', self)
        self.boxdock.setObjectName(u'Option')
        self.boxdock.setWidget(boxListContainer)
        ...
        # michaelwan 20190722 : Add box operations and dock to move faster
        self.addDockWidget(Qt.RightDockWidgetArea, self.operdock)
        ...
        self.operdock.setFeatures(self.dock.features() ^ self.dockFeatures)
        self.shapedock.setFeatures(self.dock.features() ^ self.dockFeatures)
        self.boxdock.setFeatures(self.dock.features() ^ self.dockFeatures)
    ...
    def print_info(self, win):
        print('%d %d %d %d' %(
            win.geometry().x(), win.geometry().y(),
            win.contentsRect().width(), win.contentsRect().height()
        ))
    ...
    def point(self):
        if(self.pointButton.isChecked()):
            self.canvas.object_type = 'point'

    def line(self):
        if(self.lineButton.isChecked()):
            self.canvas.object_type = 'line'

    def square(self):
        if(self.squareButton.isChecked()):
            self.canvas.object_type = 'bndbox'

    def circle(self, item= None):
        if(self.circleButton.isChecked()):
            self.canvas.object_type = 'circle'

    def polygon(self, item= None):
        if(self.polygonButton.isChecked()):
            self.canvas.object_type = 'polygon'
    ...
    def loadLabels(self, shapes):
        s = []
        for label, object_type, points, line_color, fill_color, difficult in shapes:
            shape = Shape(label=label, object_type=object_type)
            for x, y in points:
    ...
    def saveLabels(self, annotationFilePath):
        ...
        def format_shape(s):
            return dict(label=s.label,
                        ...
                        # michaelwan 20190724 add object_type
...
def get_main_app(argv=[]):
    ...
    print('%d %d %d %d ' %(app.desktop().availableGeometry().width(), app.desktop().availableGeometry().height(),
                      app.desktop().screenGeometry().width(), app.desktop().screenGeometry().height()
                       ))
    print('%d %d %d %d %d %d %d %d ' %(win.x(), win.y(), 
