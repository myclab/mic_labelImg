...
class Canvas(QWidget):
    ...
    def print_info(self, event, shapes):
        points = []
        r = event.rect()
        points.append((r.x(), r.y(), r.width(), r.height()))
        c = QCursor.pos()
        ...
        print(QTime.currentTime().toString(Qt.DefaultLocaleLongDate), pointscontainsPoint)

    def paintEvent(self, event):
        if not self.pixmap:
            return super(Canvas, self).paintEvent(event)

        #self.print_info(event, self.shapes)
        p = self._painter
        p.begin(self)
        ...
        for shape in self.shapes:
            if (shape.selected or not self._hideBackround) and self.isVisible(shape):
                ...
                shape.paint(p, shape.object_type, False)

        if self.current:
            self.current.paint(p, self.object_type, self.drawing())
            self.line.paint(p, self.object_type, self.drawing())

        if self.selectedShapeCopy:
            self.selectedShapeCopy.paint(p, self.selectedShapeCopy.object_type, False)
